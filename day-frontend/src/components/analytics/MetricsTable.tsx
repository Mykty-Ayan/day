...
const columns: { key: SortField; label: string; format: (v: number) => string }[] = [
  ...
  { key: 'adr', label: 'ADR', format: (v) => `$${Number(v).toFixed(0)}` },
  { key: 'revpar', label: 'RevPAR', format: (v) => `$${Number(v).toFixed(0)}` },
  { key: 'profit', label: 'Profit', format: (v) => `$${v.toLocaleString()}` },
  { key: 'occupancy_rate', label: 'Occupancy', format: (v) => `${Number(v).toFixed(1)}%` },
  { key: 'avg_stay_duration', label: 'Avg Stay', format: (v) => `${Number(v).toFixed(1)}d` },
  { key: 'total_bookings', label: 'Bookings', format: (v) => v.toString() },
]
  ...
  if (properties.length === 0) {
    return (
      <div className="flex items-center justify-center py-12 text-sm text-gray-400">
        No property data for the selected period
      </div>
    )
  }
